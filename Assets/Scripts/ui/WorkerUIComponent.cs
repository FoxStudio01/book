...
public class WorkerUIComponent : MonoBehaviour
{
    ...
    List<WorkerUIComponentRow> row = new List<WorkerUIComponentRow>();
    WorkerUIComponentRow title;
    ...
    void Awake()
    {
        ...
        title = SlicedSprite3.transform.Find( "title" ).GetComponent<WorkerUIComponentRow>();
        ...
        rowObject = Resources.Load<GameObject>( GameSetting.UIPath + "WorkerUIRow" );
    }
    ...
    public void updateData()
    {
        ...
        for ( int i = 0 ; i < DepartmentData.data.Count ; i++ )
        {
            ...
            DepartmentList.AddItem( str , DepartmentData.data[ i ].dID );
            if ( i == 0 )
        ...
        for ( int i = 0 ; i < PostTypeData.data.Count ; i++ )
        {
            ...
            PostTypeList.AddItem( str , PostTypeData.data[ i ].ptID );
            if ( i == 0 )
        ...
        if ( WorkerData.data.Count == 0 )
        {
            return;
        }

        if ( row.Count != WorkerData.data.Count )
        {
            for ( int i = 0 ; i < row.Count ; i++ )
            {
            ...
            for ( int i = 0 ; i < WorkerData.data.Count ; i++ )
            {
                ...
                WorkerUIComponentRow r = obj.GetComponent<WorkerUIComponentRow>();
                r.resize();
        ...
        for ( int i = 0 ; i < WorkerData.data.Count ; i++ )
        {
            row[ i ].setData( WorkerData.data[ i ] );
        }
    ...
    public void onNewDB( string d )
    {
        ...
        else
        {
            ...
            WorkerData.data.Add( p );

            GameObject obj = Instantiate<GameObject>( rowObject , Table.transform );
            obj.name = "row" + ( WorkerData.data.Count - 1 ).ToString();
            WorkerUIComponentRow r = obj.GetComponent<WorkerUIComponentRow>();
            r.resize();
            row.Add( r );
            r.setData( p );
    ...
    public void onNew()
    {
        ...
        phpSQL.instance.php( "CALL .INSERT_WORKER('" + editorNew.value + "', '" + department + "', '" + postType + "' , '" + Localization.Get( "worker" ) + "' )" , onNewDB );
    ...
    public void onUpdate()
    {
        for ( int i = 0 ; i < row.Count ; i++ )
        {
            if ( row[ i ].check() )
            {
                phpSQL.instance.php( "CALL .UPDATE_WORKER( '" + row[ i ].id() + "' , " +
    ...
    public void onDelete()
    {
        ...
        for ( int i = 0 ; i < row.Count ; i++ )
        {
            if ( row[ i ].check() )
            {
                phpSQL.instance.php( "CALL .DELETE_WORKER('" + row[ i ].id() + "' )" , onDeleteDB );
                needRefresh++;
            }
        }
    }
