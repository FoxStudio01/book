...
public class PostTypeUIComponent : MonoBehaviour
{
    ...
    List< PostTypeUIComponentRow > row = new List< PostTypeUIComponentRow >();
    PostTypeUIComponentRow title;
    ...
    void Awake()
    {
        ...
        title = SlicedSprite3.transform.Find( "title" ).GetComponent< PostTypeUIComponentRow >();
        ...
        rowObject = Resources.Load<GameObject>( GameSetting.UIPath + "PostTypeUIRow" );
    }
    ...
    public void updateData()
    {
        ...
        if ( PostTypeData.data.Count == 0 )
        {
            return;
        }

        if ( row.Count != PostTypeData.data.Count )
        {
            for ( int i = 0 ; i < row.Count ; i++ )
            {
            ...
            for ( int i = 0 ; i < PostTypeData.data.Count ; i++ )
            {
                ...
                PostTypeUIComponentRow r = obj.GetComponent<PostTypeUIComponentRow>();
                r.resize();
        ...
        for ( int i = 0 ; i < PostTypeData.data.Count ; i++ )
        {
            row[ i ].setData( PostTypeData.data[ i ] );
        }
    ...
    public void onNewDB( string d )
    {
        ...
        else
        {
            ...
            PostTypeData.data.Add( p );

            GameObject obj = Instantiate<GameObject>( rowObject , Table.transform );
            obj.name = "row" + ( PostTypeData.data.Count - 1 ).ToString();
            PostTypeUIComponentRow r = obj.GetComponent<PostTypeUIComponentRow>();
            r.resize();
            row.Add( r );
            r.setData( p );
    ...
    public void onNew()
    {
        ...
        phpSQL.instance.php( "CALL .INSERT_POSTTYPE('" + editorNew.value + "', '" + Localization.Get( "posttype" ) + "' )" , onNewDB );
    ...
    public void onUpdate()
    {
        for ( int i = 0 ; i < row.Count ; i++ )
        {
            if ( row[ i ].check() )
            {
                phpSQL.instance.php( "CALL .UPDATE_POSTTYPE( '" + row[ i ].id() + "' , " +
                    " '" + row[ i ].editor1.value + "' , " +
                    " '" + row[ i ].editor2.value + "'" +
                    " )" , onUpdateDB );
            }
    ...
    public void onDelete()
    {
        ...
        for ( int i = 0 ; i < row.Count ; i++ )
        {
            if ( row[ i ].check() )
            {
                phpSQL.instance.php( "CALL .DELETE_POSTTYPE('" + row[ i ].id() + "' )" , onDeleteDB );
                needRefresh++;
            }
        }
    }
